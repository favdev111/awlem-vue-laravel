...
@section('content')
    ...
    <div class="parts form container">
        <div class="main-card mb-3 card">
            <div class="card-body">
                <h5 class="card-title">{{ __('تعديل ميزة') }}</h5>
                @if (count($errors) > 0)
                ...
                @endif
                {!! Form::open(['method' => 'patch', 'action' => ['Admin\FeaturesController@update', $feature->id], 'files'
                => true]) !!}
                <div class="container">
                    <div class="form-group">
                        <label for="name">{{ __('الصورة') }}</label>

                        <div class="row">
                            <div class="col-md text-center">
                                {!! Form::file('imge', null, ['class' => 'form-control', 'placeholder' =>
                                __('الصورة')]) !!}
                            </div>
                            <div class="col-md text-center">
                                @php
                                ...
                                @endphp
                                <img id="photo" src="{{ url($link) }}" style="width: 38%;object-fit: cover;"
                    ...
                    <div class="form-group">
                        <label for="name">{{ __('عنوان الميزه') }}
                        </label>
                        <div>
                            {!! Form::text('title', $feature->title, ['class' => 'form-control', 'placeholder' => __('عنوان
                            الميزة')]) !!}
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="name">{{ __('محتوي الميزه') }}
                        </label>
                        <div>
                            {!! Form::textarea('description', $feature->description, ['class' => 'form-control', 'placeholder' => __('محتوي
                            الميزة')]) !!}
                        </div>
                    </div>
