...
@section('content')
    ...
    <div class="parts form container">
        <div class="main-card mb-3 card">
            <div class="card-body">
                <h5 class="card-title">{{ __('branch.edit_branch') }}</h5>
                @if (count($errors) > 0)
                ...
                {!! Form::open(['method' => 'patch', 'action' => ['Admin\BranchesController@update', $branch->id], 'files'
                => true]) !!}
                <div class="container">
                        ...
                        <div class="row">
                            <div class="col-md">
                                <div class="form-group">
                                    ...
                                    {!! Form::text('location_lat', $branch->location_lat, ['class' => 'form-control', 'id'
                                    => 'location_lat', 'placeholder' => __('branch.location_lat')]) !!}
                                </div>
                            </div>
                            <div class="col-md">
                                <div class="form-group">
                                    ...
                                    {!! Form::text('location_lon', $branch->location_lon, ['class' => 'form-control', 'id'
                                    => 'location_lon', 'placeholder' => __('branch.location_lon')]) !!}
                                </div>
                            </div>
                        </div>
                    </div>
                    @if ($getFrom == 'Restorant' || $getFrom == 'Super Admin')
                        <div class="form-group">
                            <label for="name">{{ __('branch.user_id') }}
                            </label>
                            <div>
                                {!! Form::select('user_id', $users, $branch->user_id, ['class' => 'form-control',
                                'placeholder' => __('branch.user_id')]) !!}

                                {!! Form::hidden('getFrom', $getFrom) !!}
                                {!! Form::hidden('restorant_id', $branch->restorant_id) !!}
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            {!! Form::select('area_id', $areas, $branch->area_id, ['class' => 'form-control', 'placeholder'
                            => __('المنطقة')]) !!}
                        </div>
                    </div>

                    @if ($getFrom == 'Super Admin')
                        <div class="form-group">
                            <label for="name">{{ __('branch.restorant_id') }}
                            </label>
                            <div>
                                {!! Form::select('restorant_id', $restorants, $branch->restorant_id, ['class' =>
                                'form-control', 'placeholder' => __('branch.restorant_id')]) !!}
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            {!! Form::text('name', $branch->name, ['class' => 'form-control', 'placeholder' =>
                            __('branch.name')]) !!}
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            {!! Form::text('location_name', $branch->location_name, ['class' => 'form-control',
                            'placeholder' => __('branch.location_name')]) !!}
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-5 text-center ">
                            <div class="form-group">
                                <div>
                                    {!! Form::text('open_from', $branch->open_from, ['id' => 'open_from', 'class' =>
                                    'form-control', 'placeholder' => __('branch.open_from')]) !!}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-5 text-center ">
                            <div class="form-group">
                                <div>
                                    {!! Form::text('open_to', $branch->open_to, ['id' => 'open_to', 'class' =>
                                    'form-control', 'placeholder' => __('branch.open_to')]) !!}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        {!! Form::select('open_dayes[]', $week, $hisDays, ['id' => 'week-id', 'class' => 'form-control',
                        'multiple']) !!}
                    </div>
                    <div class="form-group">
                        ...
                        <div>
                            {!! Form::textarea('description', $branch->description, ['class' => 'form-control',
                            'placeholder' => __('نبذه عن الفرع')]) !!}
                        </div>
                    </div>
                    <div class="form-group">
                        <label style="width:150px" for="name">{{ __('يوجد سفرة') }}</label>
                        {!! Form::checkbox('eat_in_branch', null, $branch->eat_in_branch, ['class' => 'form-control',
                        'data-toggle' => 'toggle', 'data-onstyle' => 'primary']) !!}
                    </div>
                    <div class="form-group">
                        <label style="width:150px" for="name">{{ __('أمكانية التوصيل للسيارة') }}</label>
                        {!! Form::checkbox('delever_to_car', null, $branch->delever_to_car, ['class' => 'form-control',
                        'data-toggle' => 'toggle', 'data-onstyle' => 'primary']) !!}
                    </div>
            ...
            <script>
                var galleryUploader = new qq.FineUploader({
                    ...
                    session: {
                        endpoint: "{{ url('./getuploadedImages/' . $branch->id) }}"
                    },
                    thumbnails: {
                        placeholders: {
